...
---
## 7. Cloud Migration & Scaling
If you choose to move away from local hosting:

### Static Cloud (S3 + CloudFront)
- **Frontend**: Deploy the `dist` folder to an S3 bucket configured for static website hosting.
- **Media**: Store files in S3 and use AWS Lambda to handle the authentication and streaming logic.
- **Cost Warning**: Be mindful of S3 Egress fees (Data Transfer Out).

### Always-Free VPS (Oracle Cloud)
- Use an Oracle Cloud "Always Free" ARM instance (4 OCPUs, 24GB RAM, 200GB Block Storage).
- Install Node.js and `pm2` to keep the server running 24/7.
- This effectively replaces your laptop with a professional cloud data center for $0/month.

### Recommended Hardware for 24/7 Hosting
- **Device**: Raspberry Pi 4 Model B (4GB RAM)
- **Storage**: 32GB MicroSD (OS) + External SSD (Media)
- **Power**: Official 15W USB-C PSU
- **Cooling**: Passive heatsink case or active fan.
